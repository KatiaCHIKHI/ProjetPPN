...
void find_cliques_pivot(dict& hash, set<string> r, set<string> p, set<string> x, set<vector<string>>& cliques) {
    if (p.empty() && x.empty()) {
        cliques.insert(vector<string>(r.begin(), r.end()));
    }
    else {
...
set<vector<string>> find_cliques(Graph g) {
    ...
    for (int i =0; i < degen_order.size(); i++) {
        set<string> neighs;
        string v = degen_order[i];
...
vector<vector<string>> bron_kerbosch(Graph g, vector<string> degen_order){
